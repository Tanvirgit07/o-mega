    ...
    <>
      {existingProduct ? (
        <div className="flex self-start items-center justify-center gap-2 py-2 mb-2">
          <button disabled={existingProduct.quantity! <=1} onClick={() => {
              dispatch(decreaseQuantity(product?.id));
              toast.success(
                "Quantity decreased successfully!"
              );
            }} className="bg-[#f7f7f7] text-black p-2 border-[1px] border-gray-200 hover:border-sky-300 rounded-full text-sm hover:bg-white duration-200 cursor-pointer disabled:text-gray-300 disabled:hover:bg-[#f7f7f7]">
            <FaMinus />
          </button>
        ...
        <button
          ...
          className="bg-transparent border border-sky-500 text-black rounded-full py-1.5 hover:bg-sky-600 hover:text-white duration-300 my-2"
        >
          Add To Cart
        </button>
      )}
